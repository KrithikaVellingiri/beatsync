...
export default function DistributorLogin() {
  ...
  const handleBackendGoogleLogin = async (idToken: string) => {
    try {
      const res = await fetch("http://localhost:5000/api/auth/google", {
        ...
        body: JSON.stringify({ token: idToken, role: "owner" }),
      });
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.success && data.data.user.role === "owner") {
        ...
        setError("This account is not a distributor account.");
      } else {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // userInfo is an object for new versions, but let's grab idToken directly.
    ...
    >
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <View style={[styles.card, isDesktop && styles.desktopCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <Text style={[styles.title, { color: colors.text }]}>Distributor Sign in</Text>
          ...
          <View style={styles.footerRow}>
            ...
            <Pressable onPress={() => router.push("/(auth)/distributor-signup")}>
